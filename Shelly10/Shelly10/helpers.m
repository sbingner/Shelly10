...
int execprog(const char *prog, const char* args[]) {
    ...
    if (file_exists("/shelly") != 0) {
        mkdir("/shelly", 0755);
    }
    if (file_exists("/shelly/logs") != 0) {
        mkdir("/shelly/logs", 0755);
    }
    
    const char *logfile = [NSString stringWithFormat:@"/shelly/logs/%@-%lu",
    ...
    do {
        ...
        if (ret>0)
            NSLog(@"'%s' exited with %d (sig %d)\n", prog, (char)WEXITSTATUS(status), WTERMSIG(status));
        else if (errno!=EINTR)
